...
def _build_system_instruction(name: str, description: str) -> str:
    ...
    return (
        ...
        "The generated 'rewriter' prompt MUST explicitly instruct the agent to output ONLY the rewritten "
        "query text itself — no labels or preambles (e.g. never prefix with \"Optimized search query:\" or "
        "\"Rewritten query:\"), no surrounding quotes, no explanation, and no JSON — because this text is "
        "embedded directly for retrieval and any extra text degrades search accuracy.\n"
